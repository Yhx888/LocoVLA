...
1. **反馈控制为什么能对抗扰动？** 因为反馈不关心扰动来自哪里——它只看误差。无论扰动是风、坡度还是模型误差，只要产生了误差，反馈就会产生纠正力。

2. **比例控制的两个核心限制？** (a) 稳态误差不为零（需要积分项）；(b) 增益太大会导致振荡或不稳定（需要微分项或更高级方法）。

3. **正反馈和负反馈的区别？** 负反馈减小误差（稳定），正反馈放大误差（不稳定）。在倒立摆中，正反馈会让它在毫秒内倒下。

4. **如果 kp 设为零会怎样？** 控制器输出为零，系统回到开环状态——Upkie 会在几秒内倒下。
