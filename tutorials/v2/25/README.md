...
1. **terminated 和 truncated 的区别为什么重要？** terminated 表示任务逻辑结束（机器人倒了），truncated 表示时间到但任务没结束。RL 算法计算 value 时，terminated 的后续价值为零，truncated 的后续价值需要 bootstrapping。如果混淆两者，value 估计会有偏。

2. **为什么需要 action clipping？** RL 策略（尤其是训练早期）可能输出任意大的值。没有 clipping 会导致执行器收到非法输入，仿真发散。Clipping 是环境的安全防线。

3. **观测空间为什么不直接用 qpos/qvel？** qpos 混合了不同单位（m、无量纲、rad），数值范围差异大。而且 qpos 包含四元数（4 维表示 3 个自由度），存在冗余约束。项目实际采用 15 维特征观测：从 qpos/qvel 中提取 pitch_error、base_height 等物理意义明确的特征，既消除了四元数冗余，又为每个维度设置了合理的裁剪范围。神经网络对输入尺度敏感——一个维度如果比其他维度大 100 倍，梯度会被它主导。

4. **环境的 seed 为什么重要？** 可复现性是科学实验的基本要求。同一个 seed 应该产生完全相同的初始状态和随机扰动。如果你的环境用了 `np.random` 而不是 `self.np_random`，seed 就不会生效。
