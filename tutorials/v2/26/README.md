...
1. **稀疏奖励和密集奖励的权衡？** 稀疏奖励目标明确但学习慢，密集奖励学习快但容易引入偏差。实践中通常从密集奖励开始，逐步减少 shaping 项。

2. **为什么终止条件必须独立于奖励？** 如果"奖励很低"就终止 episode，RL 策略会学到"避免低奖励状态"而不是"从低奖励状态恢复"。这会导致策略保守——一有危险就"自杀"（让 terminated=True），而不是尝试恢复。

3. **奖励缩放对学习率的影响？** 奖励太大→梯度太大→需要更小的学习率；奖励太小→梯度消失→需要更大的学习率。通常把奖励归一化到 [-1, 1] 或 [0, 1] 范围。

4. **怎样检测奖励漏洞？** 观察策略的异常行为：如果策略学到了你没预期的"奇怪"行为但奖励很高，说明奖励函数有漏洞。常见漏洞：不动、振荡、利用仿真 bug。
