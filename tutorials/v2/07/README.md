...
1. **URDF 和 MJCF 的核心区别是什么？** URDF 只描述运动学结构（关节、连杆），MJCF 还包含动力学（质量、惯量）、接触（碰撞几何、摩擦）和控制（执行器、传感器）。

2. **为什么审计不能只检查语法？** 一个语法正确的 MJCF 可以有物理上错误的参数——比如轴方向反了、质量设为 0、关节范围太大。语法检查器不管物理语义。

3. **position actuator 和 motor actuator 的本质区别？** position 是闭环的（内部有 PD 控制器），motor 是开环的（直接输出力矩）。如果你用 motor 做位置控制，你需要自己写 PD 逻辑。

4. **如果 nq 和配置不一致，第一个可见症状是什么？** 代码中用索引访问 `qpos[10]` 时可能越界，或者访问到了错误的关节。更隐蔽的是：索引没越界但读到的物理量不是你以为的那个。
