...
<!-- upkie-qa:15-q1 -->
每个极点 `s = sigma + j*omega` 对应一个模态 `e^(sigma*t) * (振荡因子)`。实部 `sigma` 决定幅值包络的增减：负实部指数衰减（越负衰减越快，时间常数 `1/|sigma|`），正实部指数发散，零实部持续等幅振荡。虚部 `omega` 决定振荡角频率：虚部为零则单调无振荡，虚部越大振荡越密。两者合起来就能不看完整曲线直接预判响应形态：例如一对共轭极点 `-2±j5` 表示以约 5 rad/s 振荡、包络按 `e^(-2t)` 衰减的响应。
<!-- /upkie-qa -->

2. 为什么过零不能证明稳定？

<!-- upkie-qa:15-q2 -->
曲线穿过零只说明某个时刻瞬时值为零，不说明幅值包络在衰减。不稳定的振荡发散（正实部、非零虚部的共轭极点）同样会反复过零，只是每次振幅都比上一次更大；只截取前几个周期看，它和衰减振荡很像。判据必须是包络趋势（或直接看极点实部符号）：取相邻峰值比较，比值小于 1 才是衰减。实验上还要看足够长的时间窗口，避免把慢发散模态误判为稳定。
<!-- /upkie-qa -->

3. `zeta=0.7` 为什么几乎没有共振峰？

<!-- upkie-qa:15-q3 -->
二阶系统频率响应出现共振峰的条件是 `zeta < 1/sqrt(2) ≈ 0.707`，峰值大小为 `1/(2*zeta*sqrt(1-zeta^2))`。当 `zeta=0.7` 时恰好贴在临界值上，代入公式得峰值仅约 1.0002，放大不到 0.02%，工程上可忽略。物理直觉：阻尼已经强到能在能量在惯性和弹性之间往复积累前就把它耗散掉，任何频率的激励都不会被显著放大。这也是 `zeta≈0.7` 被当作经典设计目标的原因之一：响应足够快（超调约 5%）又几乎没有频域放大风险。
<!-- /upkie-qa -->

4. 连续极点稳定但仿真发散时先查什么？

<!-- upkie-qa:15-q4 -->
先查离散化步长与积分方法。连续域稳定只保证数学解衰减，仿真器用有限步长 `dt` 离散积分，离散后的等效极点可能跑出单位圆：对显式欧拉，稳定条件大致是 `dt < 2*|sigma|/(sigma^2+omega^2)`，快模态（大 `|sigma|` 或高频 `omega`）对步长最敏感。验证方法：把 `dt` 减半重跑，若发散消失就是数值问题。其次再查建模与仿真的不一致：线性化模型漏掉的非线性（大偏角、接触切换）、执行器限幅、延时等，都可能让"纸面稳定"的系统在仿真中发散。
<!-- /upkie-qa -->

5. 局部稳定为什么不等于轮足机器人全局安全？

<!-- upkie-qa:15-q5 -->
极点分析建立在平衡点附近的线性化模型上，结论只在线性化有效区域（如俯仰 ±15°~20°）内成立。真实的轮足机器人还有一堆线性模型没覆盖的机制：大偏角时 `sin(theta)` 非线性让重力力矩比模型预测增长得更猛；轮端力矩限幅 `±1 N*m` 意味着存在一个可恢复偏角上限，超过了再优秀的增益也无力矩可用；打滑或离地会直接切换动力学结构。所以局部稳定只回答"平衡点附近能不能站住"，全局安全还需要吸引域估计、饱和分析和接触监控共同支撑，不能拿极点在左半平面当安全证书。
<!-- /upkie-qa -->
