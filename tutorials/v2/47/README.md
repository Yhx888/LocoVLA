...
<!-- upkie-qa:47-q1 -->
最关键的假设是：源码能被 `py_compile` 编译且能被 `ast` 解析——这是所有后续分析（覆盖率、复杂度、重复率、静态告警）的前提。如果这个假设失效（比如某个文件有语法错误），第一个可观测信号是 `syntax_errors > 0`，`review_pass` 立即翻为 0，不需要等其他指标。本关正文的观察示例就是验证这个机制：故意删除一个冒号制造语法错误后，`syntax_errors` 从 0 变 1，`review_pass` 立即翻转——语法错误是最严重的问题，文件根本无法导入，其他所有分析都没有意义。这个设计的优先级很清晰：`syntax_errors == 0` 是硬性门槛，无论 pytest-cov 是否可用都必须满足；覆盖率、重复率、静态告警是软性门槛，只在工具可用时才检查。面试时的判断框架：代码评审指标的优先级是"能编译 > 有测试 > 测试覆盖够 > 代码整洁"，这个顺序对应"正确性 > 可验证性 > 可维护性"的工程价值排序。常见误区是把所有指标同等对待——`static_warnings` 上升 50% 和 `syntax_errors` 从 0 变 1 的严重程度完全不同，前者是可读性问题，后者是代码根本无法运行。另一个常见误区是"review_pass=1 就等于代码质量好"——本关正文明确声明 `engineering_47.json` 的 `passed=true` 只表示本地自动条件满足，不能替代外部人工答辩，自动评审能检查结构，不能检查设计是否合理。
<!-- /upkie-qa -->

2. 为什么 `review_pass` 在 pytest-cov 未安装时降级为「无语法错误」？有哪些可替代方案？

<!-- upkie-qa:47-q2 -->
降级的逻辑是"退而求其次保证最基本的可编译性"。pytest-cov 可用时，`review_pass` 的完整门槛是四个条件的逻辑与：`coverage >= 50%` AND `duplicate_percent <= 50%` AND `static_warnings <= 100` AND `syntax_errors == 0`。pytest-cov 不可用时，覆盖率无法度量（返回 0.0），如果仍然要求 `coverage >= 50%`，`review_pass` 会永远为 0，评审系统完全失效——这比没有评审更糟，因为它给出"永远不通过"的错误信号。降级为只检查 `syntax_errors == 0` 保留了最有价值的检查（代码能编译），同时诚实地标注"覆盖率分析跳过"，而不是假装覆盖率是 0%。门槛值的设计也有依据：教学项目含大量仿真脚本、ROS2 编排入口和样板代码（写 result/存 log/画图/写 portfolio），覆盖率 50% 和重复率 50% 是对这类项目的合理预期，不是对生产代码的标准。替代方案有两种：一是用纯 `coverage.py`（不依赖 pytest-cov 插件）度量覆盖率，消除工具依赖，但需要额外的配置工作；二是把门槛改为"测试全部通过"（`pytest` 退出码 0），这比覆盖率更直接，但不能区分"测试通过但只覆盖了 10% 代码"和"测试通过且覆盖了 80% 代码"。面试时的判断框架：评审门槛的降级策略应该是"保留最有价值的检查，诚实标注缺失的部分"，而不是"全部放弃"或"假装没问题"。常见误区是"门槛越高越好"——门槛太高会导致评审永远不通过，开发者会绕过评审而不是改进代码。
<!-- /upkie-qa -->

3. 你能用哪三份证据证明结果可复现？

<!-- upkie-qa:47-q3 -->
三份证据对应"人类可读、机器可读、课程验收"三个层次。第一份是代码评审报告 `outputs/reports/code_review_47.md`：用表格列出每个文件的语法错误、未用导入、长行、函数数、类数、最大嵌套深度，摘要表给出模块数、覆盖率、平均复杂度、重复比例、静态告警数、评审通过；这是给人类审查者看的，回答"哪些文件有问题、问题是什么"。第二份是指标契约 `outputs/reports/code_review_47_metrics.json`：包含 git_commit 和所有量化指标（`coverage_percent`、`avg_complexity`、`duplicate_percent`、`static_warnings`、`syntax_errors`、`review_pass`），是机器可读的精确数值；让别人能在相同代码版本下重跑 `python scripts/run_engineering_lab_47.py` 得到相同指标，这是可审计性的基础。第三份是结果契约 `outputs/results/engineering_47.json`：包含 `passed=true/false`、`pass_conditions`（通过条件列表）和 `checks`（每项检查的结果），是课程验收系统（`course_checkpoint.py --chapter 47`）读取的文件；本关正文明确声明 `passed=true` 只表示本地自动条件满足，不能自证真人答辩通过。三份证据的分工：报告回答"代码质量怎么样"，指标契约回答"具体数字是多少"，结果契约回答"是否满足课程验收条件"。面试时的判断框架：如果面试官问"你怎么证明代码质量没有退化"，答案是固定 git_commit + 指标契约可重算 + 每次代码变更后重跑评审。常见误区是只交报告——报告里的数字是自我声明，指标契约和结果契约才能让别人独立验证。
<!-- /upkie-qa -->

4. 如果 `static_warnings` 突然上升 50%，你先查模型、数据、控制还是部署？为什么？

<!-- upkie-qa:47-q4 -->
先查部署，具体说是查最近的代码变更引入了什么。`static_warnings = 未用导入数 + 长行数`，这两个分量都是纯文本层面的问题，与模型、数据、控制律的物理逻辑完全无关——控制律改了增益不会让某行代码超过 120 字符，训练数据变了不会让某个 import 变成未使用。最可能的根因按概率排序：第一，新增依赖或合并代码引入了长行（比如从外部库复制了一段代码，行宽风格不同）；第二，自动生成的代码（比如 ROS2 的消息头文件、protobuf 生成文件）被意外纳入了评审范围；第三，大段复制粘贴导致重复率上升（虽然重复率是独立指标，但复制粘贴通常同时引入长行和未用导入）。排查步骤：先看评审报告 `code_review_47.md` 的文件级表格，找到 `warning_count` 最高的文件；再看那些文件的最近 git 变更（`git log --oneline -5 -- <文件>`）；最后判断是应该修复代码（删除未用导入、拆分长行）还是应该把生成文件排除出评审范围。面试时的判断框架：静态指标退化的排查顺序是"哪个文件 → 哪次提交 → 是代码问题还是工具配置问题"，不需要任何物理或算法知识。常见误区是"static_warnings 上升就去调控制代码"——控制代码的变更几乎不会影响静态指标，除非有人在控制文件里加了一行很长的注释。另一个常见误区是"static_warnings 不重要可以忽略"——本关的门槛是 `static_warnings <= 100`，超过这个值 `review_pass` 会翻为 0，直接影响课程验收。
<!-- /upkie-qa -->
