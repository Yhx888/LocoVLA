...
<!-- upkie-qa:10-q1 -->
摩擦锥是法向力和切向力之间的比值约束：接触点的切向力不能超过 `mu * F_n`，否则接触点开始滑动。在 3D 中把这个约束画在 `F_t1-F_t2-F_n` 空间里，合法接触力的集合正好是一个以法向为轴的圆锥，所以叫「摩擦锥」。对轮式机器人的实际意义是：轮端力矩能转化成的地面推力有上限，上限由法向力（大致是体重的一半）乘摩擦系数决定，超出这个上限的力矩只会让轮子空转。
<!-- /upkie-qa -->

2. 打滑时第一个可观测信号是什么？

<!-- upkie-qa:10-q2 -->
轮子角速度远大于 `v_body / r_wheel`。不打滑时轮子滚动满足纯滚动约束，轮速×半径应该等于车体前进速度；一旦打滑，轮子在原地空转，两者的比值立刻拉开。只要在控制循环里持续监控 `wheel_omega * r / v_body` 这个比值，就能实时检测打滑——这也是汽车 ABS/牵引力控制系统的基本原理。
<!-- /upkie-qa -->

3. 为什么仿真中法向力不完全等于 mg/2？

<!-- upkie-qa:10-q3 -->
两个原因：数值积分的误差，以及 MuJoCo 接触模型的「软约束」特性——接触不是刚性的几何约束，而是允许微小穿透的弹簧-阻尼系统，接触力随穿透深度动态调整。每次步进后接触力都可能有微小振荡，只要均值接近 mg/2、振荡幅度在百分之几以内，这就是正常现象，不是 bug。反过来，如果法向力系统性偏离 mg/2 很远，才说明质量配置或接触参数有问题。
<!-- /upkie-qa -->

4. 摩擦系数设错了对控制器的影响？

<!-- upkie-qa:10-q4 -->
如果仿真中 mu 远大于真实值，控制器（尤其是 RL 策略）会以为自己有更大的地面推力裕量，学会用激进的加减速策略；部署到真实机器人上，同样的力矩会突破真实摩擦锥，轮子打滑、机身失控摔倒。反方向设错（仿真 mu 偏小）则会让策略过于保守。因为真实地面的 mu 本身就不确定（地毯、瓷砖、水渍各不相同），所以工程上的解法不是把 mu 调「准」，而是在训练时随机化 mu 让策略对它鲁棒——这就是域随机化（关卡 29）要解决的问题之一。
<!-- /upkie-qa -->
