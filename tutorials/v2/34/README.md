...
<!-- upkie-qa:34-q1 -->
因为它是安全关键命令，失败的代价与其他命令不在同一个量级。“前往红色目标”解析错了，后果是任务失败；“停止”响应慢了或被忽略，后果可能是碰撞、伤人或设备损坏。所以工程实现上必须给它一条与普通流程分离的快速通道：普通指令要经过“解析→感知→规划→控制”的完整链路（约 133 ms，见 q4），而停止命令在解析层被识别后直接短路到安全层，毫秒级生效，不等感知和规划。这背后的设计原则是：安全路径的可靠性不能依赖任何可能出错或延迟的环节。感知可能卡帧、规划可能陷入异常分支、神经网络可能输出不确定结果，但停止命令的响应路径上只有确定性的硬编码逻辑——它的正确性可以被单独测试、单独证明。这与真实工业系统的急停按钮（E-stop）同构：急停从不经过 PLC 的普通逻辑，而是硬线直达断电回路。把停止做成“又一条普通指令”是语言接口机器人最常见的设计错误。
<!-- /upkie-qa -->

2. 为什么不用 LLM 做语言解析？

<!-- upkie-qa:34-q2 -->
可以用，但要分清层级。LLM 的两个硬伤决定它不适合安全关键路径：一是延迟，一次推理 100 ms 到 1 s，而且分布长尾不可预测，无法给出硬实时保证；二是不确定性，同一输入可能产生不同输出，甚至可能被提示注入或幻觉引导出危险指令，而安全认证要求行为可枚举、可验证。确定性小模型（规则匹配或小型分类器）正好相反：延迟毫秒级且稳定，相同输入永远得到相同输出，全部分支可以被测试覆盖。正确的分工是沿 32 关的分层思路：LLM 适合做高层任务分解（把“去厨房把杯子拿来”拆成子目标序列），这层对延迟宽容、错误可被下游审查；而安全关键的指令识别（尤其是停止）必须由确定性模块把守。即使未来 LLM 延迟降到可接受，不确定性问题依然存在：安全论证需要的是“可证明的行为边界”，而不是“大概率正确”。所以本关的架构里，LLM 如果出现，只会在任务层之上，而解析器和安全层永远保持确定性。
<!-- /upkie-qa -->

3. 解析出错了怎么办？

<!-- upkie-qa:34-q3 -->
安全层兜底。即使解析器把“去悬崖”解析成“向前移动”，安全层也会在机器人接近危险边界时拦截——它不看指令语义，只看物理状态是否越界。这是分层架构的核心优势：每一层都独立提供安全保障，不把全部赌注押在任何单一环节的正确性上。把这个思路展开成工程方法就是“纵深防御”：解析层用置信度阈值拒绝模棱两可的输入（听不懂就追问而不是猜），规划层检查目标是否在合法工作区域内，安全层监控物理状态并拥有最终否决权，控制层再加力矩限幅。四道防线各自独立，攻破全部才会出事故。设计时的关键约束是失效独立性：各层的失效原因不能相关——如果解析器和安全层共用同一个神经网络的输出，那它们会同时被同一个幻觉骗过，等于只有一道防线。所以安全层必须用与上游完全不同的信息源（直接读传感器的物理状态）和完全不同的判断逻辑（硬编码阈值）。验收时也应分层注入故障分别测试：故意送错误解析结果，验证安全层确实拦得住。
<!-- /upkie-qa -->

4. 端到端延迟是多少？

<!-- upkie-qa:34-q4 -->
按链路逐段相加：解析 1 ms + 感知 100 ms + 规划 30 ms + 控制 2 ms ≈ 133 ms，即从说完话到机器人开始动作约 133 ms。这个数字本身不难算，面试中真正拉开差距的是对它的解读。第一，各段量级差异巨大：感知占了近 75%，所以优化延迟应该优先压缩感知（降分辨率、硬件加速、降帧率），而不是优化已经只有 1 ms 的解析器——先找瓶颈再动手是性能工程的基本素养。第二，133 ms 是“新指令生效”的延迟，不是平衡控制的延迟：底层控制器在整个期间仍以 100 Hz 自主维持平衡，这是 32 关分层设计的直接好处。第三，安全命令走快速通道不受这个链路约束：停止命令解析后直达安全层，响应在毫秒级（见 q1），所以“普通指令 133 ms”与“停止命令毫秒级”并存不矛盾。第四，这是纸面估算，实机上各段延迟有分布和抖动，工程验收应用监控日志统计实测分布（均值、P95、最大值），40 关的实时性实验会把这套方法落地。
<!-- /upkie-qa -->
