...
<!-- upkie-qa:19-q1 -->
加速度计对运动加速度敏感——它测的是比力（重力与运动加速度的叠加），而不是纯重力方向。当 Upkie 静止时，加速度计读数指向重力，可以反解出俯仰角；但一旦机器人加速前进或制动，运动加速度混入读数，算出的"重力方向"就是错的，角度估计随之出错。而且平衡控制正好需要在机器人加减速时给出准确姿态，这恰是加速度计最不可信的时刻。所以加速度计只适合提供长期（低频）参考，短时动态必须交给陀螺仪。
<!-- /upkie-qa -->

2. 为什么不能只用陀螺仪？

<!-- upkie-qa:19-q2 -->
陀螺仪测的是角速度，角度要靠积分得到，而陀螺仪存在偏差（bias）：积分后偏差误差随时间线性增长。即使偏差只有 0.001 rad/s，10 秒后角度误差就有 0.01 rad（约 0.57 度），1 分钟后约 3.4 度——对平衡控制而言这已经是致命漂移。陀螺仪短时间内非常准（高频好），但没有绝对参考，无法自己纠正漂移；必须用加速度计提供的重力方向作为长期锚点把积分误差拉回来。两者缺陷正好互补，这就是"互补"滤波名字的由来。
<!-- /upkie-qa -->

3. alpha 的物理意义是什么？

<!-- upkie-qa:19-q3 -->
alpha 决定了两个传感器的分工频率边界，即截止频率 `f_c ≈ (1-alpha)/(2*pi*dt)`（精确式为 `-ln(alpha)/(2*pi*dt)`）：高于 f_c 的信号成分主要信陀螺仪（短时动态），低于 f_c 的成分主要信加速度计（长期参考）。alpha 越接近 1，截止频率越低，越依赖陀螺仪，响应快但漂移纠正慢；alpha 越小越信加速度计，抗漂移但容易被运动加速度污染。选 alpha 本质上是在"陀螺漂移速度"和"运动加速度干扰频率"之间划分界线。
<!-- /upkie-qa -->

4. 互补滤波的局限是什么？

<!-- upkie-qa:19-q4 -->
它假设加速度计的长期平均等于重力方向——这在长时间持续加速（如匀速圆周运动、长坡持续加速）时不成立，此时长期锚点本身就是歪的，滤波器会把估计稳稳地拉向错误方向。此外 alpha 是手调的固定权重，不随工况自适应：同一个 alpha 无法同时适配静止、慢速和激烈机动。解决这两个问题需要 Kalman 滤波（关卡 20）：用噪声协方差显式建模两个传感器的不确定性，让融合权重随状态和时间自动调整；互补滤波正是 Kalman 滤波在特定噪声协方差下的特例。
<!-- /upkie-qa -->
