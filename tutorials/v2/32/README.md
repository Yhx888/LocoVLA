...
1. **为什么不能让高层直接控制底层？** 频率不匹配（高层 10 Hz 远慢于 100 Hz 动作更新）、语义不匹配（高层说"去那里"，底层需要"N*m 力矩"）、安全不匹配（高层不知道力矩限制）。500 Hz 是 MuJoCo 物理积分频率，不是新的策略动作频率。

2. **安全层为什么必须在规划层和控制层之间？** 因为安全约束是物理的（俯仰角、力矩），不是语义的（任务目标）。安全层必须理解物理状态才能判断指令是否安全。

3. **端到端延迟是多少？** 若按最坏等待估算，任务处理 100 ms + 规划 30 ms + 等待下一次动作更新最多 10 ms，约为 140 ms。物理积分每 2 ms 推进一步，但同一动作要保持 5 个物理子步，不能把 2 ms 当成新动作的响应周期。

4. **如果感知出错（检测到错误的目标颜色），系统会怎样？** 规划层会向错误目标移动，但安全层仍然保证不翻倒。最坏情况：机器人移动到错误的地方但保持平衡——不会造成物理损坏。
