...
1. position actuator 和 motor actuator 的核心区别？

<!-- upkie-qa:09-q1 -->
position actuator 内部自带 PD 控制器，`ctrl` 输入是目标角度（单位 rad），MuJoCo 每步自动把角度误差换算成力矩；motor actuator 直接输出力矩，`ctrl` 输入就是力矩值（单位 N·m）。两者的输入量纲完全不同：Upkie 的髋关节 kp=40、膝关节 kp=60，如果把目标角度当力矩发给 motor（或反过来），量级会差几十倍，机器人要么痉挛要么痲痹。接口层必须把「哪些通道是角度、哪些是力矩」写进契约并用断言钉死。
<!-- /upkie-qa -->

2. ctrl 值被裁剪时你怎么知道？

<!-- upkie-qa:09-q2 -->
MuJoCo 不会报错，它默默把超出 `ctrlrange` 的值裁到边界。发现裁剪的办法是对比 `data.ctrl`（你请求的）和 `data.actuator_force`（实际产生的）：如果 ctrl 请求 2.0 但 ctrlrange 是 [-1, 1]，实际力矩只有 1.0。工程实践上应该在控制循环里主动检测「请求值碰到边界」并记入日志——长时间饱和意味着控制器要的力矩物理上给不出来，这通常是增益过大或机械设计不匹配的信号。
<!-- /upkie-qa -->

3. 观测数据和 qpos/qvel 的关系？

<!-- upkie-qa:09-q3 -->
当前配置通过 `sensor_contract` 直接从 qpos/qvel 读取状态，XML 里没有定义传感器，所以观测是状态的确定性函数。但如果将来添加 IMU 传感器（加速度计、陀螺仪），它们的数据是额外的物理量，不能从 qpos/qvel 直接推导：加速度计读数包含重力分量和科里奥利力，静止时读到的不是 0 而是 9.81 m/s²。区分「从状态派生的观测」和「独立的传感器物理量」，是后面做观测空间设计和 sim-to-real 时的关键前提。
<!-- /upkie-qa -->

4. 为什么轮端的 ctrlrange 是 [-1, 1] N*m？

<!-- upkie-qa:09-q4 -->
因为这是真实轮端电机的力矩上限。超过这个范围的力矩在物理上无法产生，仿真中设置同样的裁剪是为了忠实反映这个硬件约束。如果仿真里放开限制，控制器（尤其是 RL 策略）会学会依赖现实中不存在的大力矩，部署到真机时直接失控——这是典型的 sim-to-real gap 来源。把执行器限制建进仿真，等于在训练阶段就把硬件能力边界告诉控制器。
<!-- /upkie-qa -->
