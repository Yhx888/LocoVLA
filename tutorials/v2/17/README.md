...
<!-- upkie-qa:17-q1 -->
是在代价函数 `J = int(x^T Q x + u^T R u) dt` 意义下的最优——不是绝对最优，而是针对你选择的 Q 和 R 的最优。换一组 Q/R 就得到不同的"最优"增益：把俯仰角的 Q 权重加大，控制器会更激进地纠姿态、付出更大力矩；把 R 加大，则更省力矩但收敛变慢。所以 LQR 把"调增益"问题转化成了"调代价权重"问题，最优性只在你定义的衡量标准内成立；面试时说"LQR 是最优控制"而不补充"在给定二次代价下"，是典型的不严谨。
<!-- /upkie-qa -->

2. ARE 的解 P 必须满足什么条件？

<!-- upkie-qa:17-q2 -->
P 必须是对称正半定矩阵。因为 `x^T P x` 的物理含义是从状态 x 出发的最优剩余代价，代价不可能为负；如果 P 有负特征值，说明代价函数可以无限减小（问题无界），通常是系统不可控导致的——某个不稳定模态输入够不着，代价只能发散。这正是 16 章可控性检查是 Riccati 设计结构前提的原因：求解器（如 `scipy.linalg.solve_continuous_are`）报错或返回带负特征值的 P 时，应先回去检查可控性和 Q/R 的半定性，而不是硬用结果。
<!-- /upkie-qa -->

3. LQR 比手动调 PD 好在哪里？

<!-- upkie-qa:17-q3 -->
三点：(a) 系统化计算多变量增益——四状态系统手调需要同时凑 4 个以上强耦合的增益，LQR 一次 Riccati 求解全部给出；(b) 在模型准确的前提下保证最优性和闭环稳定；(c) 调参接口直观：Q/R 权重直接对应"更在乎哪个状态、多省力矩"，比盲调增益可解释得多。但代价是假设更强：LQR 依赖线性模型和二次代价，对大偏差、非线性效应和模型误差无能为力；而 PD 不需要显式模型，在模型拿不准时反而更鲁棒。两者是"有模型用模型"与"无模型靠经验"的权衡，不是单纯的优劣关系。
<!-- /upkie-qa -->

4. LQR 失效的第一个信号是什么？

<!-- upkie-qa:17-q4 -->
状态偏差大到线性化假设不成立（比如俯仰角超过 30 度）：此时真实重力力矩比线性模型预测的更大，LQR 增益不再最优，可能无法恢复。可观察的征兆依次是：控制量长时间顶在饱和边界（`±1 N*m`）、状态偏差不降反升、最后姿态发散。处置策略是分层的：小偏差用 LQR，接近边界时切换到非线性控制或 RL 策略，超出可恢复域则触发安全降级（如 18 章的俯仰误差阈值清零命令），而不是指望同一组线性增益包打天下。
<!-- /upkie-qa -->
