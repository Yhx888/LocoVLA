...
1. 深度图和 RGB 图的分辨率为什么通常不同？

<!-- upkie-qa:33-q1 -->
因为两者的成像原理完全不同。RGB 传感器是被动接收可见光，像素密度只受光学和制造工艺限制，如今手机传感器轻松做到千万像素级；而深度传感器要主动发射信号再解算距离——结构光需要投射编码图案并匹配形变，ToF 需要测量每个像素的光飞行时间，两种方式的物理分辨率都受投射器功率、基线长度和解算算力限制，通常比 RGB 低一到两个量级。分辨率不同带来的工程后果是必须做配准（registration）：深度图和 RGB 图由两个物理位置不同的传感器拍摄，同一个物体在两张图里的像素坐标不一致，需要用两者的内参和相对外参把深度图重投影到 RGB 坐标系，才能回答“这个红色像素的深度是多少”。配准过程会引入插值误差和遮挡空洞（RGB 能看到但深度相机被遮挡的区域），这些都是下游检测算法必须容忍的噪声来源。在 MuJoCo 仿真里 RGB 和深度由同一个渲染相机生成、天然对齐，这是仿真与实机的又一个隐藏差异。
<!-- /upkie-qa -->

2. 深度图在什么情况下失效？

<!-- upkie-qa:33-q2 -->
三类典型失效场景，都源于深度传感器依赖主动光信号这一物理前提。第一，透明与反射物体：玻璃让红外光直接穿透（测到的是玻璃后面的物体），镜面把光反射到别处（测到的是镜中虚像的距离），两者都产生“看似合理却完全错误”的深度值，比缺失更危险。第二，室外强光：阳光中的红外成分远强于传感器自己发射的信号，结构光图案被淹没，深度图大面积缺失。第三，距离过远：主动光能量随距离平方衰减，超出量程后信噪比不足，深度值噪声急剧增大直至不可用。关键的工程启示是：这些失效在 MuJoCo 仿真中全部不存在——仿真深度直接从几何渲染得到，没有光学环节，所以永远干净完美。这正是感知层的 sim-to-real gap：在仿真里验证过的视觉管道，到实机上首先要面对的就是这些仿真从未呈现过的失效模式。对照 29 关的思路，应对方法同样是两条：要么在仿真中注入深度噪声与空洞模型做随机化，要么在下游算法里设计对缺失鲁棒的处理逻辑（如空洞填充、多帧融合）。
<!-- /upkie-qa -->

3. 从像素到世界坐标需要哪些信息？

<!-- upkie-qa:33-q3 -->
四样信息，对应两次坐标变换。第一步从像素坐标到相机坐标，需要：(a) 像素坐标 (u,v)——目标在图像中的位置；(b) 该像素的深度 d——没有深度时一个像素只能确定一条射线，深度把射线上的无穷多个点确定为一个；(c) 相机内参 (fx, fy, cx, cy)——焦距和主点描述了针孔模型的投影几何，反投影公式是 `X=(u-cx)*d/fx, Y=(v-cy)*d/fy, Z=d`。第二步从相机坐标到世界坐标，需要 (d) 相机外参——相机在世界坐标系中的位置和姿态（旋转矩阵加平移向量）。四者缺一不可，且各有各的误差来源：内参靠标定板标定，标定不准会让边缘像素的反投影系统性偏斜；外参在机器人上由机械安装位置加实时姿态估计给出，机身俯仰估计误差会直接传导成目标位置误差——这把 19/20 关的状态估计质量和感知精度直接耦合在了一起：Upkie 上俯仰角估计每偏 1°，2 m 外目标的高度定位就偏约 3.5 cm。所以定位精度预算应该把四个环节的误差分别量化，而不是只盯着检测算法本身。
<!-- /upkie-qa -->

4. 为什么不用纯视觉 SLAM 代替深度相机？

<!-- upkie-qa:33-q4 -->
两者解决的问题和代价结构不同，不是替代关系。纯视觉 SLAM 通过多帧之间的特征匹配和三角化估计深度，好处是只需一个普通相机，但代价明显：计算量大（特征提取、匹配、优化都要算力）；初始化慢（单目需要足够视差才能三角化，开机后要先移动一段才有深度）；单目还有尺度不确定性（只能恢复相对深度，绝对尺度需要额外信息）；低纹理场景（白墙）特征匹配直接失效。深度相机则单帧直接给出带绝对尺度的深度，实时性好、不依赖纹理，适合近场避障这种对延迟敏感的任务；代价是 q2 列举的光学失效场景和有限量程。所以工程实践中两者互补：SLAM 负责大尺度建图和定位（利用它的全局一致性），深度相机负责近场障碍检测（利用它的实时性和绝对尺度）。选型时的判断框架是回到任务需求：延迟预算、精度需求、场景特性（室内/室外、纹理丰富度）和算力预算共同决定组合方式，而不是存在一个普适的“更先进”方案。
<!-- /upkie-qa -->
