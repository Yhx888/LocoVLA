...
<!-- upkie-qa:03-q1 -->
读作「从 body 坐标系到 world 坐标系的旋转」：右下标 `body` 是输入坐标系（坐标目前表达在机身系下），左上标 `world` 是输出坐标系（变换后表达在世界系下）。这个命名约定的好处是可以像分数约分一样做「下标相消」检查：`p_world = R_world_body @ p_body`，相邻的两个 `body` 相消，剩下 `world`，一眼就能看出乘法方向对不对；如果写成 `R_world_body @ p_world`，下标对不上，说明方向用反了。
<!-- /upkie-qa -->

2. 为什么点需要加平移，而速度向量通常不加？

<!-- upkie-qa:03-q2 -->
点描述的是「位置」，它依赖坐标系原点在哪里，所以完整变换是 `p_world = R @ p_body + t`，其中 `t` 是机身原点在世界系下的位置。速度、角速度、力这类「自由向量」只有方向和大小，与原点无关——把坐标系原点平移 100 米，一个向东 1 m/s 的速度仍然是向东 1 m/s，所以只旋转不平移：`v_world = R @ v_body`。判断标准就是：这个量平移坐标系后会不会变？会变的是点，不变的是向量。
<!-- /upkie-qa -->

3. 为什么逆旋转可以使用转置？前提是什么？

<!-- upkie-qa:03-q3 -->
因为旋转矩阵是正交矩阵：列向量两两垂直且长度为 1，满足 $R^T R = I$，所以 $R^{-1} = R^T$，转置比通用求逆既快又数值稳定。前提是：这个矩阵确实是有效的旋转矩阵（正交且 $\det(R)=+1$）。如果矩阵经过长时间数值积累已经不再正交，或者根本不是旋转矩阵（含缩放、剪切），那么 $R^T$ 就不等于 $R^{-1}$，用转置会引入错误。所以工程中要定期检查正交性或重新正交化。
<!-- /upkie-qa -->

4. `det(R)=-1` 为什么值得警惕？

<!-- upkie-qa:03-q4 -->
行列式为 -1 的正交矩阵不是旋转，而是「旋转 + 镜像反射」：它会把右手坐标系翻成左手坐标系。物理世界里刚体怎么转都不会镜像，所以真实旋转矩阵的行列式永远是 +1。一旦检查到 $\det(R)=-1$，说明构造过程有 bug：可能是某一列符号写反、两列顺序弄错，或者从外部数据读入时轴定义不一致。继续用它做变换，机器人的左右会被静默地颠倒。
<!-- /upkie-qa -->

5. 角度单位错用“度”时，哪项检查可能仍通过，哪项现象会先异常？

<!-- upkie-qa:03-q5 -->
正交性检查仍会通过：把 30 当成 30 rad 代进 `cos/sin` 构造的矩阵依然是合法的旋转矩阵（$R^T R = I$、$\det(R)=1$ 都成立），因为它确实是「某个角度」的旋转，只是不是你想要的那个角度。先异常的是几何结果本身：变换后的坐标与手算/作图预期对不上（30 rad ≈ 1718°，相当于转了近 5 圈后停在一个看似随机的方向）。这就是为什么除了矩阵性质检查，还必须用已知答案的具体数值例子（如绕 z 转 30° 后 x 轴去向）做对齐验证。
<!-- /upkie-qa -->

6. 最小奇异值接近 0 为什么意味着某个方向难以观测？

<!-- upkie-qa:03-q6 -->
奇异值分解把矩阵看成「在各个方向上的拉伸倍数」：最小奇异值对应被拉伸得最少的那个输入方向。若最小奇异值接近 0，说明状态在这个方向上变化时，观测输出几乎不变——信号被压到和噪声同一量级，从输出反推这个方向的状态就要除以这个接近 0 的数，噪声会被巨幅放大（条件数 $\sigma_{max}/\sigma_{min}$ 变得很大）。工程上的含义是：要么增加/改进传感器让这个方向可观测，要么在估计器中接受这个方向的高不确定性。
<!-- /upkie-qa -->

7. 为什么特征向量不能用逐元素数值固定其正负号？

<!-- upkie-qa:03-q7 -->
特征向量定义里就带着符号自由度：若 $Av = \lambda v$，则 $-v$ 同样满足 $A(-v) = \lambda(-v)$，两者都是合法的特征向量。不同的线性代数库、同一个库的不同版本、甚至输入矩阵的微小扰动，都可能返回相反符号的结果。因此测试里不能断言「第一个分量等于 0.8」这种逐元素固定值，而应断言与符号无关的性质：如 $\|Av - \lambda v\| \approx 0$、向量长度为 1、或先把符号归一化（规定最大分量为正）后再比较。
<!-- /upkie-qa -->
