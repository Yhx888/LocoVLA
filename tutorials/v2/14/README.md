...
<!-- upkie-qa:14-q1 -->
倒立摆的重力力矩是 `m*g*l*sin(theta)`，方向与偏角同号：机身往哪边倾，重力就产生把它推得更歪的力矩。线性化后动力学里这一项对应正反馈（状态矩阵中系数为正），与普通单摆的回复力矩（负反馈，把摆拉回最低点）正好相反。因此偏角一旦非零就会指数发散，必须靠轮端主动力矩持续对抗；这也解释了为什么平衡控制器一旦关闭，Upkie 必倒无疑。
<!-- /upkie-qa -->

2. `sin(theta)≈theta` 的平衡点和有效范围是什么？

<!-- upkie-qa:14-q2 -->
这个近似是在直立平衡点 `theta=0` 处的一阶泰勒展开，只在平衡点附近有效。定量看相对误差 `(theta - sin(theta))/sin(theta)`：10°（≈0.17 rad）时约 0.5%，20° 时约 2%，30° 时约 4.7%，工程上通常把 ±15°~20° 当作可接受边界。超出这个范围，基于线性模型设计的增益（PD、LQR）就不再可信——模型低估了真实重力力矩的非线性增长，控制器可能救不回来。注意平衡点选在直立不稳定点而不是下垂稳定点，这是轮足机器人建模与普通单摆的关键区别。
<!-- /upkie-qa -->

3. 为什么增加质心高度会同时改变重力力矩和惯量？

<!-- upkie-qa:14-q3 -->
质心高度 `l` 同时出现在两个物理量里：重力力矩 `m*g*l*sin(theta)` 随 `l` 线性增长（力臂变长）；绕轮轴的转动惯量约 `m*l^2` 随 `l` 平方增长。两者一起决定失稳速度：发散时间尺度正比于 `sqrt(l/g)`，所以质心越高倒得越慢（扫把比铅笔好站），但需要的纠正力矩也更大。工程含义是：改变站姿高度（如 18 章高度命令改变髋/膝关节角）后，控制增益的物理尺度也变了，不能把低姿态调好的增益直接搬到高姿态。
<!-- /upkie-qa -->

4. 接触力 `J^T lambda` 为什么不能在轮子离地时忽略？

<!-- upkie-qa:14-q4 -->
问题恰好相反：轮子离地时 `lambda=0`，接触项确实消失——但随之消失的还有整个控制通路。轮端力矩能影响机身姿态，靠的就是地面通过 `J^T lambda` 把摩擦力和法向力传递回来；一旦离地，轮子只会空转，控制器对俯仰的作用归零，基于"轮端力矩能扭回机身"建立的动力学方程整体失效。所以不能把接触项当成可有可无的小修正项忽略：必须监控接触状态（如 `wheel_contact_ratio`），接触丢失时切换到保护逻辑，而不是继续用接地模型算控制量。
<!-- /upkie-qa -->

5. 如何用小扰动实验验证力矩符号？

<!-- upkie-qa:14-q5 -->
把机器人置于直立静止状态，施加一个已知符号的小力矩（如轮端 +0.1 N·m，其余清零），只跑几十个仿真步，观察初始响应方向：记录轮角速度和俯仰角速度的符号，与模型预测对比（正力矩应使轮子正转、机身因反作用力矩向反方向俯仰）。关键要点：扰动要小且时间要短，保证系统还在线性化有效区域内，反馈控制器必须关闭（否则控制器会掩盖开环响应）；对每个执行器分别做正负两个方向。这能一次性抓出建模符号错误、执行器方向系数（如左右轮 `[1,-1]`）或传感器符号约定的不一致。
<!-- /upkie-qa -->
