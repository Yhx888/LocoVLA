...
<!-- upkie-qa:24-q1 -->
动力学模型准确。MPC 的每一步都在用 A、B 矩阵沿未来时域预测状态轨迹，再在预测上优化；如果模型与真实系统偏差大，预测就不准，优化出的"最优动作序列"也不可靠——相当于在错误地图上规划最优路线。失效时第一个可观测信号：仿真中 MPC 表现好，但实机上变差（模型-现实差距暴露）。滚动优化能部分缓解模型误差（每步用真实状态重新规划），但缓解不等于免疫。
<!-- /upkie-qa -->

2. 为什么当前接口、单位和限幅这样设计？

<!-- upkie-qa:24-q2 -->
`compute(state, reference)` 与 LQR 接口一致，方便对比脚本复用——同一套评估流程可以无缝切换两种控制器，保证对比公平。`control_limit` 对应物理电机的力矩上限（Upkie 轮端 ±1.0 N*m），把约束写进优化问题而不是事后 clip，让求解器在可行域内直接找最优（这正是 23 章强调的核心区别）。替代方案：可以用 CVXPY 等凸优化库替代 scipy，获得更好的 QP 求解性能与更清晰的约束建模语法。
<!-- /upkie-qa -->

3. 你能用哪三份证据证明结果可复现？

<!-- upkie-qa:24-q3 -->
(a) `estimation_24.json` 中的 `seed=0`——固定随机种子保证初始状态和扰动序列可重现；(b) `estimation_24.log` 中的求解统计——记录每步 QP 求解状态与耗时，可验证优化过程本身没有异常；(c) `test_mpc.py` 中的自动化测试——把验收阈值固化进 pytest，任何人重跑都能得到同样的通过/失败判定。三者分别锁定输入、过程和验收标准，缺一不可；这也是本项目"固定 seed + 日志 + pytest 门禁"实验约定的具体实例。
<!-- /upkie-qa -->

4. 如果指标退化 20%，你先查模型、数据、控制还是部署？

<!-- upkie-qa:24-q4 -->
先查模型（A、B 矩阵是否与仿真对齐），再查控制（约束设置是否与实际电机一致）。理由：MPC 的性能上限由预测模型决定，模型错了后面全部白跑，所以排查优先级最高；约束配置错误（如限幅写大）会让优化器规划出物理上执行不了的动作，是第二常见原因。数据和部署在本章纯仿真设定下不涉及（没有训练数据集，也没有跨平台部署）；到了 38~43 章工程部署阶段，排查清单才需要把部署（时序、精度、通信）加回来。
<!-- /upkie-qa -->
