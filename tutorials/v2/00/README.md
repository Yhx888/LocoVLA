...
1. **本关最关键的假设是什么？** 假设你的环境（Python 3.11 + MuJoCo）已正确配置。失效时的第一个信号是 `import mujoco` 报错或版本号不匹配。

2. **为什么 nq > nv？** 四元数用 4 个数表示 3 自由度的旋转，多出的 1 维是归一化约束。面试时画一个球面就能解释清楚。

3. **为什么关卡顺序不可调换？** 因为每个关卡的输出是下一个关卡的输入。跳过仿真直接学 PPO，你训练出来的策略会在一个物理错误的模型上运行。

4. **你能用哪三份证据证明环境可用？** (a) `python --version` 输出 3.11.x；(b) `import mujoco` 成功并输出版本号；(c) `scripts/01_check_model.py` 正确输出 nq=13, nv=12, nu=6。
