...
<!-- upkie-qa:13-q1 -->
`clip` 只限制送到执行器的输出，不限制控制器内部的积分状态。饱和期间误差持续存在，积分器仍每步累加 `Ki*e*dt`，把内部状态推到远超实际需要的水平（实验中裸积分到 2.34，而抗饱和版本为 0）。目标回零后，这笔"积分债"必须先被反向误差一点点抵消，输出才会改变方向，于是出现明知目标已回零却继续满力矩输出的危险恢复过程。也就是说 `clip` 保证了输出安全，但没有保证控制器状态健康——windup 是状态问题，不是输出问题。
<!-- /upkie-qa -->

2. 条件积分在什么情况下允许积分器反向消除积累？

<!-- upkie-qa:13-q2 -->
条件积分的拒绝条件是两条同时成立：`u_raw` 已超出限幅，且 `e * u_raw > 0`（本次误差会把输出推向更深饱和）。反过来，只要任一条不成立就接受 `I_candidate`：一是输出解除饱和（`|u_raw| <= u_max`），积分器完全恢复正常；二是虽然仍饱和但误差已反向（`e * u_raw < 0`），此时新误差是在把积分往回拉、帮助退出饱和，必须允许它累加，否则积分器会被"冻结"在高位无法消除积累。这正是条件积分比"饱和期间直接停积分"更精细的地方。
<!-- /upkie-qa -->

3. D 项为什么容易放大编码器噪声？

<!-- upkie-qa:13-q3 -->
微分项按 `(e[k]-e[k-1])/dt` 计算，分母是很小的 `dt`（本章 0.01 s）。编码器量化噪声让相邻两拍的测量值跳动一个最小分辨率，这个跳动本身很小，但除以 0.01 后被放大 100 倍，再乘 `Kd` 直接进入输出，表现为高频动作抖动。而且噪声频率越高，差分放大越严重——微分本质上是高通滤波器，增益随频率线性上升。工程上的缓解办法是对测量值先做低通滤波、用测量微分代替误差微分（避免目标突变引入的微分踢），或降低 `Kd`。
<!-- /upkie-qa -->

4. 为什么本章检查 release 时刻，而不要求积分峰值始终为零？

<!-- upkie-qa:13-q4 -->
因为积分器存在的意义就是累积误差来消除稳态误差，正常工作时它本来就应该非零；要求峰值始终为零等于禁用积分。真正危险的是解除饱和（release）时刻的积分残留：目标已经回零、系统本该立即恢复，控制器却因为背着积分债继续输出错误方向的满力矩。所以验收指标看 `protected_integral_at_release`（0.0）对比 `naive_integral_at_release`（2.34），以及恢复段 IAE 改善比（12.6 倍）——检查的是"该恢复时能不能立刻恢复"，而不是"过程中积分有多大"。
<!-- /upkie-qa -->

5. 如果轮端正负力矩上限不同，抗饱和逻辑怎样修改？

<!-- upkie-qa:13-q5 -->
把对称的 `clip(u_raw, -u_max, u_max)` 换成非对称 `clip(u_raw, u_min, u_max)`（例如 `u_min=-0.6, u_max=1.0`），条件积分的判据也要按方向分开写：当 `u_raw > u_max` 且 `e > 0`（误差推向正饱和更深）时拒绝积分；当 `u_raw < u_min` 且 `e < 0` 时拒绝积分；其余情况照常累加。不能继续用 `e * u_raw > 0` 这种依赖对称性的简写，否则在靠近较小一侧上限时会误判。若使用反算抗饱和，则公式 `I += Kb*(u - u_raw)*dt` 天然兼容非对称限幅，只需保证 `u` 用非对称 clip 计算。
<!-- /upkie-qa -->
