...
1. 为什么需要 4 个数表示 3 自由度旋转？

<!-- upkie-qa:08-q1 -->
因为用 3 个参数表示 SO(3)（三维旋转群）必然存在奇异点，这是拓扑学定理，不是工程实现问题——欧拉角的万向节锁就是这个奇异点的具体表现。四元数用 4 个参数加一个归一化约束（模长等于 1）绕开了这个问题：参数空间是四维单位球面，任何姿态都能平滑、无奇异地表示。代价是多一个冗余维度，以及每次积分后需要重新归一化。
<!-- /upkie-qa -->

2. 万向节锁在 Upkie 中会发生吗？

<!-- upkie-qa:08-q2 -->
不会。两个原因：Upkie 的根部自由关节用四元数表示姿态，没有奇异点；腿部的铰链关节每个只有 1 个自由度，根本用不到 3 个角度的组合表示。但要注意：如果你在外部分析代码里把四元数转成欧拉角来看姿态，当躯干接近垂直（pitch 接近 ±90°）时 yaw 和 roll 会混合，曲线会突变跳变——这不是仿真错了，而是欧拉角表示本身的奇异性在作祟。
<!-- /upkie-qa -->

3. 四元数的半角从哪来？

<!-- upkie-qa:08-q3 -->
来自四元数的旋转公式：旋转一个点用的是三明治式的 `q * p * q^{-1}`，q 在左右各乘一次。每次乘 q 贡献 theta/2 的旋转，两次合起来才是完整的 theta。所以四元数的分量写作 `(cos(theta/2), sin(theta/2)*轴)`：绕 z 轴转 180° 的四元数是 `(0, 0, 0, 1)`（半角 90°），而不是直觉以为的 `(cos180°, ...)`。写代码时忘记除以 2 是四元数相关 bug 的头号来源。
<!-- /upkie-qa -->

4. `qvel[3:6]` 是角速度，它的参考坐标系是什么？

<!-- upkie-qa:08-q4 -->
体坐标系（body frame），不是世界坐标系。这意味着当 Upkie 旋转时，角速度的三个分量始终相对于躯干当前的前后/左右/上下方向：机器人原地转身 90° 后，同样的「前翻」角速度在体坐标系下分量不变，但在世界坐标系下方向已经完全不同。如果控制器把它当世界系角速度用，机器人一旦偏航旋转，反馈方向就会错乱——这和 IMU 陀螺仪的输出约定一致，正好方便 sim-to-real 对接。
<!-- /upkie-qa -->
