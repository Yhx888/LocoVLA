...
1. **`qpos` 和 `qvel` 为什么维度不同？** 四元数用 4 个数表示 3 自由度旋转，角速度只有 3 个分量。面试时画一个四元数球面就能解释。

2. **半隐式欧拉和显式欧拉的区别是什么？** 半隐式先用新速度更新位置，显式先用旧速度更新位置。半隐式是辛结构，长时间能量有界；显式会能量发散。

3. **怎样判断仿真结果是物理正确的？** 三个检查：(a) 自由落体轨迹与 `z = z_0 - 0.5*g*t^2` 一致；(b) 四元数模长保持 1.0；(c) 无外力时总能量守恒。

4. **dt 太小有什么问题？** 计算时间线性增长。如果 dt = 0.0001，仿真 10 秒需要 100000 步，每步都要解一次线性方程组 `M^{-1}F`，在复杂模型上会很慢。
